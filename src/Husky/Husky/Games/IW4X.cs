...
namespace Husky
{
    ...
    class IW4X
    {
        ...
        {
            // Found her
            printCallback?.Invoke("Found supported game: Call of Duty: Modern Warfare 2 - IW4X");
            // Validate by XModel Name
            printCallback?.Invoke($"AssetPools Address: {assetPoolsAddress:X} + 0x10");
            var math = reader.ReadInt32(assetPoolsAddress + 0x10);
            printCallback?.Invoke($"Math: {math:X}");
            var str = reader.ReadNullTerminatedString(reader.ReadInt32(reader.ReadInt32(assetPoolsAddress + 0x10) + 4));
            ...
            if (reader.ReadNullTerminatedString(reader.ReadInt32(reader.ReadInt32(assetPoolsAddress + 0x10) + 4)) == "void")
            {
                ...
                printCallback?.Invoke($"GfxMapName: {gfxMapName}");
                string mapName = reader.ReadNullTerminatedString(gfxMapAsset.MapNamePointer);
                printCallback?.Invoke($"MapName: {mapName}");
                ...
                else
                {
                    ...
                    // Build output Folder
                    string outputName = Path.Combine("exported_maps", "iw4x", gameType, mapName, mapName);
