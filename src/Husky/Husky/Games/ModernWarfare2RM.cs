...
namespace Husky
{
    ...
    public class ModernWarfare2RM
    {
        ...
        public unsafe struct Material
        {
            ...
            /// Unknown Bytes (Flags, settings, etc.)
            /// </summary>
            public fixed byte UnknownBytes1[23];
        ...
        {
            // Found her
            printCallback?.Invoke("Found supported game: Call of Duty: Modern Warfare 2 Remastered");

            // Validate by XModel Name
            var baseAddr = reader.GetBaseAddress();
            var si = reader.ReadInt64(reader.ReadInt64(baseAddr + assetPoolsAddress + (16 * 5)) + 8);
            var po = reader.ReadNullTerminatedString(0x25fe6f51f65) == "fx";
            if (po)
            {
                // Load BSP Pools (they only have a size of 1 so we don't care about reading more than 1)
                var gfxMapAsset = reader.ReadStruct<GfxMap>(reader.ReadInt64(0x25fe6f51f65 + 0xD8));
            ...
            {
                printCallback?.Invoke("Call of Duty: Modern Warfare 2 Remastered is supported, but this EXE is not.");
            }
        }
